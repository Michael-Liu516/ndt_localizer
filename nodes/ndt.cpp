...
NdtLocalizer::NdtLocalizer(ros::NodeHandle &nh, ros::NodeHandle &private_nh)
  ...
  sensor_aligned_pose_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("points_aligned", 10);
  ...
  transform_probability_pub_ =
      nh_.advertise<std_msgs::Float32>("transform_probability", 10);
  ...
  // Subscribers
  initial_pose_sub_ = nh_.subscribe("initialpose", 100,
                                    &NdtLocalizer::callback_init_pose, this);
  map_points_sub_ =
      nh_.subscribe("points_map", 1, &NdtLocalizer::callback_pointsmap, this);
  sensor_points_sub_ = nh_.subscribe("filtered_points", 1,
                                     &NdtLocalizer::callback_pointcloud, this);
...
void NdtLocalizer::timer_diagnostic() {
  ...
  while (ros::ok()) {
    ...
    if (key_value_stdmap_.count("state") &&
        key_value_stdmap_["state"] == "Initializing") {
...
void NdtLocalizer::callback_init_pose(
  ...
  if (initial_pose_msg_ptr->header.frame_id == map_frame_) {
    ...
    geometry_msgs::TransformStamped::Ptr TF_pose_to_map_ptr(
        new geometry_msgs::TransformStamped);
    get_transform(map_frame_, initial_pose_msg_ptr->header.frame_id,
                  TF_pose_to_map_ptr);

    // transform pose_frame to map_frame
    geometry_msgs::PoseWithCovarianceStamped::Ptr mapTF_initial_pose_msg_ptr(
        new geometry_msgs::PoseWithCovarianceStamped);
    tf2::doTransform(*initial_pose_msg_ptr, *mapTF_initial_pose_msg_ptr,
                     *TF_pose_to_map_ptr);
    // mapTF_initial_pose_msg_ptr->header.stamp =
    // initial_pose_msg_ptr->header.stamp;
...
void NdtLocalizer::callback_pointsmap(
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr map_points_ptr(
...
void NdtLocalizer::callback_pointcloud(
  ...
  const std::string sensor_frame =
      sensor_points_sensorTF_msg_ptr->header.frame_id;
  ...
  geometry_msgs::TransformStamped::Ptr TF_base_to_sensor_ptr(
      new geometry_msgs::TransformStamped);
  get_transform(base_frame_, sensor_frame, TF_base_to_sensor_ptr);

  const Eigen::Affine3d base_to_sensor_affine =
      tf2::transformToEigen(*TF_base_to_sensor_ptr);
  const Eigen::Matrix4f base_to_sensor_matrix =
      base_to_sensor_affine.matrix().cast<float>();

  boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>>
      sensor_points_baselinkTF_ptr(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::transformPointCloud(*sensor_points_sensorTF_ptr,
  ...
  if (!init_pose) {
    ...
    // for the first time, we don't know the pre_trans, so just use the
    // init_trans, which means, the delta trans for the second time is 0
  ...
  const double align_time =
      std::chrono::duration_cast<std::chrono::microseconds>(align_end_time -
                                                            align_start_time)
          .count() /
      1000.0;
  ...
  std::cout << "delta x: " << delta_translation(0)
            << " y: " << delta_translation(1) << " z: " << delta_translation(2)
            << std::endl;
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr sensor_points_mapTF_ptr(
      new pcl::PointCloud<pcl::PointXYZ>);
  pcl::transformPointCloud(*sensor_points_baselinkTF_ptr,
                           *sensor_points_mapTF_ptr, result_pose_matrix);
  ...
  key_value_stdmap_["seq"] =
      std::to_string(sensor_points_sensorTF_msg_ptr->header.seq);
  key_value_stdmap_["transform_probability"] =
      std::to_string(transform_probability);
  key_value_stdmap_["iteration_num"] = std::to_string(iteration_num);
  key_value_stdmap_["skipping_publish_num"] =
      std::to_string(skipping_publish_num);
...
void NdtLocalizer::init_params() {
  ...
  private_nh_.getParam("converged_param_transform_probability",
...
bool NdtLocalizer::get_transform(
  ...
  try {
    *transform_stamped_ptr =
    ...
    ROS_WARN("%s", ex.what());
    ROS_ERROR("Please publish TF %s to %s", target_frame.c_str(),
              source_frame.c_str());
...
void NdtLocalizer::publish_tf(const std::string &frame_id,
